...
`;
		...
		function UsageDock({ rpc, t, locale = "zh" }) {
			const [provider, setProvider] = (0, react.useState)("deepseek");
			...
			}, react.default.createElement("span", { className: `dpu-dot ${statusHealth}` }), react.default.createElement("span", { className: "dpu-collapsed-provider" }, display.providerLabel), react.default.createElement("strong", { className: "dpu-collapsed-headline" }, display.headline), react.default.createElement("span", {
			...
			}, tabs.map((tab) => createTab(tab, provider, provider === tab.id, () => setProvider(tab.id)))), react.default.createElement("button", {
				type: "button",
